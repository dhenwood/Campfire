...
Cisco _had_ a solution to provide a Campfire experience however it is no longer available. The following is a simplified solution to the predecesor (albeit with some additional caveats). This is made up of 4 Cisco Room Bar Pro devices and accompaning Navigator all operating independantly of each other - but the macro notifies the other 3 to perform specific tasks (see below). As these operate independantly of each other, the specific models do not matter (as an extreme example, 4 Desk Pro's could be used) - however as mentioned in the caveats below, thought needs to be put into the audio aspects of the solution (microphones and speakers) to avoid feedback loops. **NOTE:** this is not Cisco or TAC supported.
